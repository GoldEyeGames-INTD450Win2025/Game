...
func _process_release():
	...
	if is_inside_dropabale:
		for neighbour in occupying:
			neighbour.is_occupied = false
		for neighbour in to_be_occupied:
			neighbour.is_occupied = true
		occupying = to_be_occupied
		to_be_occupied = []
...
func _on_area_2d_mouse_entered() -> void:
	if not Global.is_dragging and not Input.is_action_pressed("left_click") and not Input.is_action_pressed("right_click"):
		draggable = true
